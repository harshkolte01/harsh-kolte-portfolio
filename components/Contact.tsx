...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus('success');
      setMessage('Thank you. Your message has been sent successfully. I will get back to you within 24 hours. If you do not hear from me, please reach out on LinkedIn.');
      ...
      setStatus('error');
      setMessage('The form could not be sent right now. Please email me directly or reach out on LinkedIn and I will reply as soon as possible.');
      setTimeout(() => {
        setStatus('idle');
        setMessage('');
      }, 8000);
  ...
  return (
    <section id="contact" aria-labelledby="contact-heading" className="py-24 px-6 relative" ref={containerRef}>
      <div className="max-w-4xl mx-auto">
        <div className="contact-header text-center mb-16 opacity-0 translate-y-10">
          ...
          <h2 id="contact-heading" className="text-4xl md:text-5xl font-display font-bold text-slate-200 mb-6 drop-shadow-lg">Get In Touch</h2>
          <p className="text-slate-400 max-w-lg mx-auto text-lg leading-relaxed">
            I am open to full stack engineering roles, freelance builds, and AI product collaborations. If you need help with React, Node.js, APIs, dashboards, or web application architecture, let&apos;s talk.
          </p>
        </div>
        ...
        <div className="contact-grid grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
          ...
          <div className="space-y-8">
            <div className="contact-item opacity-0 glass-card p-8 rounded-2xl hover:border-accent-cyan/30 transition-all duration-300 hover:shadow-[0_0_30px_rgba(100,255,218,0.1)] group">
              ...
              <address className="space-y-6 not-italic">
                <a href={`mailto:${siteConfig.email}`} className="flex items-center gap-4 text-slate-400 hover:text-accent-cyan transition-all duration-300 group/link">
                  ...
                  <span className="font-mono text-sm group-hover/link:-translate-y-0.5 transition-transform">{siteConfig.email}</span>
                </a>

                <a href={siteConfig.githubUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 text-slate-400 hover:text-accent-cyan transition-all duration-300 group/link">
                  <div className="p-3 rounded-full bg-navy-800/80 group-hover/link:bg-accent-cyan/10 transition-colors border border-white/5">
                ...
                <a href={siteConfig.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 text-slate-400 hover:text-accent-cyan transition-all duration-300 group/link">
                  <div className="p-3 rounded-full bg-navy-800/80 group-hover/link:bg-accent-cyan/10 transition-colors border border-white/5">
                ...
                <a href={siteConfig.instagramUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 text-slate-400 hover:text-accent-cyan transition-all duration-300 group/link">
                  <div className="p-3 rounded-full bg-navy-800/80 group-hover/link:bg-accent-cyan/10 transition-colors border border-white/5">
          ...
          <form onSubmit={handleSubmit} className="contact-item opacity-0 glass-card p-8 rounded-2xl space-y-5 hover:border-accent-cyan/30 transition-all duration-300 hover:shadow-[0_0_30px_rgba(100,255,218,0.1)]">
            ...
            {message && (
              <div
                aria-live="polite"
                className={`p-4 rounded-lg text-sm font-mono leading-relaxed ${
                  status === 'success'
